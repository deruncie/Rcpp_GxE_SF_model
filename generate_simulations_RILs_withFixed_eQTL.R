...
simulate_data = function(
  ...
  E = X %*% B + Z1 %*% E_A1 + E_R
  F = X %*% B_F + Z1 %*% F_A1 + F_R
  Y = matrix(1,n,1) %*% matrix(mu,1,p) + t(apply(cisGenotypes,1,'*',cis_effects)) + F %*% t(Lambda) + E %*% diag(sqrt(sigma2_Resid))
  
  return(list(Y=Y,F=F,F_A1 = F_A1, cis_effects = cis_effects))
}
