fast_BSFG_sampler = function(BSFG_state,n_samples) {
	...
	invert_aPXfA_bDesignDesignT      = run_variables$invert_aPXfA_bDesignDesignT  
	...
	for(i in start_i+(1:n_samples)){
	 ...
	 # -- save sampled values (after thinning) -- #
		if( (i-burn) %% thin == 0 && i > burn) {
			...
			Posterior = save_posterior_samples( sp_num,Posterior,Lambda,F,F_b,F_a,B,W,E_a,delta,F_h2,resid_Y_prec,E_a_prec,W_prec)
