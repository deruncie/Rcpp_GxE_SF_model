...
//phenotype residuals
	...
	vec tau_e = 1/(1-F_h2);
	mat S = chol(Lambda.t() * Lmsg + diagmat(tau_e)).t();
	mat Meta = trans(solve(S,trans(Y_tilde * Lmsg + sweep_times(X * F_b + Z_1 * F_a,2,tau_e))));
	...
	mat F = trans(solve(trans(S),trans(Meta + Zlams)));
...
// 	vec tau_e = 1/(1-F_h2);
// 	vec tau_u = 1/F_h2;
// 	mat b = Design_U.t() * sweep_times(F,2,tau_e);
...
// 				// F_a.col(j) = rbind(qr.solve(Cov,t(X_f) %*% F.col(j)*tau_e(j)) + qr.solve(chol(Cov),rnorm(b_f)),matrix(0,nr = r-b_f, nc = 1))
// 			} else{
// 				F_a.col(j) = zeros(r);
// 			}
// 		// } else if(tau_e(j) == inf) {
