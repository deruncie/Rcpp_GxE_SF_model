...
#     X_W    fixed-effect design matrix for transcripts
#     Z_W    lines-replicates incidence matrix: n x r
...
# ----------------------- #
    ...
    r       = setup$r   
    r       = setup$r   
    L_act   = setup$error_factor_Lambda
...
#     X = [ones(size(X_f,1),1) X_f]
    ...
    #determine if a design matrix (X_f) exists (is loaded from setup.mat). If
    #not, make a dummy X-matrix with no columns.
    if(! 'X_f' %in% ls() || is.null(X_f)) {
        X_f=matrix(0,nr = n,nc = 0)
    }
    stopifnot(nrow(X_f) == n)
    X_f = X
    b_f = ncol(X_f)
    ...
    priors$b_X_prec            =   1/n*t(X) %*% X
    priors$b_Xf_prec           =   1/n*t(X_f) %*% X_f
...
# ----------------------------- # 
   ...
   # --- transcript-level model
    ...
    F_b = matrix(rnorm(b_f*k,0,1),b_f,k, byrow = T)
    ...
    F = X_f %*% F_b + Z_1 %*% F_a + matrix(rnorm(k*n,0,sqrt(1-F_h2)),nr = n,nc = k, byrow = T)    
...
#     [U,S,~]          = svd(XZ*blkdiag(1e6*eye(b_f),A)*XZ')
 ...
 #    #fixed effects + random effects 1 for factors
 #    #diagonalize mixed model equations for fast inversion: 
 ...
 #    result = GSVD_2_c(cholcov(bdiag(priors$b_Xf_prec,Ainv)),cholcov(Design2))
    ...
    #   s2 = diag(result$S)^2
    #   )
    # invert_aPXfA_bDesignDesignT$Design_U = Design %*% invert_aPXfA_bDesignDesignT$U
...
# ----------------------------- #
    ...
    run_variables = list(
            ...
            # invert_aPXfA_bDesignDesignT      = invert_aPXfA_bDesignDesignT,
