...
update_k = function( F,Lambda,F_b,F_a,F_h2,Lambda_prec,Plam,delta,tauh,Z_W,Lambda_df,delta_2_shape,delta_2_rate,b0,b1,i,epsilon,prop ) {
...
save_posterior_samples = function( sp_num,Posterior,Lambda,F,F_b,F_a,B,W,E_a,delta,F_h2,resid_Y_prec,E_a_prec,W_prec) {
	...
	if(length(Lambda) > nrow(Posterior$Lambda)){
		...
		Posterior$F_b    = rbind(Posterior$F_b,    	matrix(0,nr = length(F_b) 	-nrow(Posterior$F_b),	nc = sp))
	...
	Posterior$F_b[1:length(F_b),sp_num] = c(F_b)
